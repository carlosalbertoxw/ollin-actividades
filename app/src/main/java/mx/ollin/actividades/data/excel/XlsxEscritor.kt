...
class XlsxEscritor(private val hojas: List<Hoja>) {
    ...
    private fun coreXml(): String {
        val ahora = DateTimeFormatter.ISO_INSTANT.format(
            Instant.now().let { Instant.ofEpochSecond(it.epochSecond) }
        )
        return """<?xml version="1.0" encoding="UTF-8" standalone="yes"?>""" +
            """<cp:coreProperties xmlns:cp="http://schemas.openxmlformats.org/package/2006/metadata/core-properties" """ +
            """xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:dcterms="http://purl.org/dc/terms/" """ +
            """xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">""" +
            "<dc:title>Actividades</dc:title><dc:creator>Ollin</dc:creator>" +
    ...
    private fun estilosXml(): String = buildString {
        ...
        append("""<numFmt numFmtId="165" formatCode="hh:mm"/>""")
        append("""<numFmt numFmtId="166" formatCode="#,##0"/>""")
        append("""<numFmt numFmtId="167" formatCode="#,##0.00"/>""")
        append("""<numFmt numFmtId="168" formatCode="0.0%"/>""")
        append("</numFmts>")
    ...
    private fun celdaXml(fila: Int, columna: Int, celda: Celda): String {
        ...
        return when (celda) {
            ...
            is Celda.Hora ->
                """<c r="$ref"$s><v>${numero(Ooxml.aFraccion(celda.valor))}</v></c>"""
